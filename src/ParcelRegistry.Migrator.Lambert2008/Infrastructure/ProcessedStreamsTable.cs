namespace ParcelRegistry.Migrator.Lambert2008.Infrastructure
{
    ...
    /// <summary>
    /// Bookkeeping of the parcel streams that have been converted, so a restart resumes instead of starting
    /// over. Lives in its own schema next to the event store, in the same database, so a stream and its
    /// bookkeeping cannot drift apart across databases.
    /// </summary>
    /// <remarks>
    ...
    /// convert nothing. Both modes still keep their timings, which is what the table is for.
    /// </remarks>
...
CREATE TABLE {Table}(
    ...
    [GeometryByteCount] [int] NOT NULL,
    [WasConverted] [bit] NOT NULL,
    ...
    CONSTRAINT [PK_Lambert2008ProcessedStreams] PRIMARY KEY CLUSTERED ([Id] ASC, [IsDryRun] ASC)
        ...
        /// afterwards — cost against geometry size, the slowest streams, percentiles — instead of having to be
            ...
            StreamConversionResult result)
        {
            try
            {
                ...
                await connection.ExecuteAsync(
                    $@"INSERT INTO {Table} (Id, IsDryRun, ParcelId, GeometryByteCount, WasConverted, LoadMilliseconds, DispatchMilliseconds, IsPageCompleted, ProcessedAt)
                       VALUES (@Id, @IsDryRun, @ParcelId, @GeometryByteCount, @WasConverted, @LoadMilliseconds, @DispatchMilliseconds, 0, SYSDATETIMEOFFSET())",
                    new
                    {
                        Id = internalId,
                        IsDryRun = _dryRun,
                        ...
                        WasConverted = result.NeedsConversion,
