            ...
            StreamConversionResult result)
        {
            try
            {
                ...
                await connection.ExecuteAsync(
                    $@"INSERT INTO {Table} (Id, ParcelId, GeometryByteCount, WasConverted, LoadMilliseconds, DispatchMilliseconds, IsPageCompleted, ProcessedAt)
                       VALUES (@Id, @ParcelId, @GeometryByteCount, @WasConverted, @LoadMilliseconds, @DispatchMilliseconds, 0, SYSDATETIMEOFFSET())",
                    new
                    {
                        Id = internalId,
                        ParcelId = parcelId,
