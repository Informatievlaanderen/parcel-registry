namespace ParcelRegistry.Tests.ApiTests.Sync
{
    ...
    public class GivenObjectCrsFilter
    {
        ...
        private static byte[] GeometryWithoutSrid()
            => GeometryHelpers.ValidGmlPolygon.ToExtendedWkbGeometryWithoutSrid();
        ...
        private static async Task<string> WriteFeed(byte[] geometry, string? objectCrs)
        {
            ...
            using (var xmlWriter = XmlWriter.Create(sw, new XmlWriterSettings { Async = true, Indent = true, Encoding = sw.Encoding }))
            {
                ...
                await writer.WriteParcel(
                    new OptionsWrapper<ResponseOptionsV2>(new ResponseOptionsV2 { Naamruimte = "https://data.vlaanderen.be/id/perceel" }),
                    formatter,
                    "category",
                    parcel,
        ...
        /// <summary>
        /// A geometry persisted before the event store wrote EWKB carries no SRID, is read as Lambert 72, and
        /// on the default path travels to the GML builder as the very bytes that were persisted — so the
        /// builder's own reader has to cope with SRID-less input too, not just the one in
        /// <c>ToRequestedCrs</c>.
        /// </summary>
        [Fact]
        public async Task WhenPersistedWithoutSrid_ThenItIsReadAsLambert72()
        {
            var feed = await WriteFeed(GeometryWithoutSrid(), objectCrs: null);

            FirstXOfObjectGeometry(feed).Should().BeApproximately(FirstXLambert72, 0.01);
        }

        /// <summary>
        /// The same bytes on the transformed path, where they are re-serialized as EWKB before the builder
        /// sees them.
        /// </summary>
        [Fact]
        public async Task WhenPersistedWithoutSrid_ThenRequesting3812Converts()
        {
            var feed = await WriteFeed(GeometryWithoutSrid(), objectCrs: "3812");
