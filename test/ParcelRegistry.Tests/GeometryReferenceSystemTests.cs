namespace ParcelRegistry.Tests
{
    ...
    public class GeometryReferenceSystemTests
    {
        ...
        /// <summary>
        /// What the event store holds for parcels written before it wrote EWKB: plain WKB carrying no SRID,
        /// the leftover of NTS's <c>AsBinary()</c>. Those are Lambert 72 by definition.
        /// </summary>
        private static Geometry SridlessPolygon =>
            new WKBReader { HandleSRID = true }.Read(
                new WKBWriter { Strict = false, HandleSRID = false }.Write(GeometryHelpers.ValidPolygon));
        ...
        /// <summary>
        /// A geometry whose bytes carried no SRID transforms like any other Lambert 72 one, and comes out
        /// labelled — so the transformation also fills in the missing label. Nothing reaches this method
        /// unlabelled today, because every caller reads through <c>WKBReaderFactory.CreateForEwkb</c>, which
        /// falls back to the Lambert 72 reader and stamps 31370. This pins the behaviour anyway, because the
        /// dispatch is on coordinates and must not start depending on the label.
        /// </summary>
        [Fact]
        public void AnUnlabelledGeometryIsTreatedAsLambert72()
        {
            var sridless = SridlessPolygon;

            // -1, not 0 and not 31370: a plain WKBReader on the default geometry services reports no SRID
            // that way. IsSupported(-1) is false, so this is a geometry GuardPolygon would reject — and
            // ToReferenceSystem still handles it, because it reads the coordinates rather than the label.
            sridless.SRID.Should().Be(-1);

            var transformed = sridless.ToReferenceSystem(SystemReferenceId.SridLambert2008);
        ...
        /// <summary>
        /// And asking for the system it is already in labels it rather than leaving it unlabelled, which is
        /// the same rule <c>ToReferenceSystem</c> applies to a mislabelled geometry.
        /// </summary>
        [Fact]
        public void AnUnlabelledGeometryAskedForLambert72IsLabelled()
        {
            var sridless = SridlessPolygon;
            var originalX = sridless.Coordinate.X;

            var result = sridless.ToReferenceSystem(SystemReferenceId.SridLambert72);

            result.SRID.Should().Be(SystemReferenceId.SridLambert72);
            result.Coordinate.X.Should().Be(originalX);
        }
